...
export function FFCBookingForm({ pageTitle, variant = 'default', packageName, defaultPackageSlug, onClose }: FFCBookingFormProps) {
      ...
      <Card className={`${variant === 'modal' ? '' : 'shadow-lg'}`}>
        <CardContent className="py-12 text-center">
          <div className="w-16 h-16 rounded-full mx-auto mb-4 flex items-center justify-center bg-amber-100">
            <CheckCircle className="h-8 w-8 text-rose-900" />
          </div>
  ...
  return (
    <Card className={`${variant === 'modal' ? 'border-0 shadow-none' : 'shadow-lg'} ${variant === 'hero' ? 'bg-white/60 backdrop-blur-md border-white/20' : ''} overflow-hidden`}>
      {variant !== 'modal' && (
        <div className={`${variant === 'hero' ? 'bg-gradient-to-r from-rose-900/95 to-amber-700/95' : 'bg-gradient-to-r from-rose-900 to-amber-700'} p-4 text-white`}>
      ...
      {variant === 'modal' && onClose && (
        <div className="flex justify-between items-center p-4 border-b">
          <div>
            <CardTitle className="text-xl text-rose-900">Reserve Now</CardTitle>
      ...
      <CardContent className="p-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label className="flex items-center gap-2 text-sm font-medium">
                <Gift className="h-4 w-4 text-rose-900" />
                Your Moment *
              </Label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="occasionDate" className="flex items-center gap-2 text-sm font-medium">
                <Calendar className="h-4 w-4 text-rose-900" />
                Select Date *
              </Label>
              <Input
                id="occasionDate"
            ...
            <div className="space-y-2">
              <Label className="flex items-center gap-2 text-sm font-medium">
                <Clock className="h-4 w-4 text-rose-900" />
                Preferred Time *
              </Label>
          ...
          <Button
            ...
            className="w-full bg-gradient-to-r from-rose-900 to-amber-700 hover:from-rose-950 hover:to-amber-800 text-white py-5 text-base font-semibold mt-2"
          >
            {isSubmitting ? (
              <>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button className={`bg-gradient-to-r from-rose-900 to-amber-700 hover:from-rose-950 hover:to-amber-800 text-white ${className}`}>
